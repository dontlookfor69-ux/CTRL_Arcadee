...
SPRITE_DIR = os.path.join(_BASE, "sprites")
...
class TexsheetList(data_types.DynamicLoadDict):
    def fetch(self, name):
        path = os.path.join(_BASE, 'decompilation', 'texture', f'{name}.png')
...
class TextureList(data_types.DynamicLoadDict):
    def fetch(self, name):
        path = os.path.join(_BASE, 'decompilation', 'texpage', f'{name}.json')
        with open(path) as f:
            texdata = json.load(f)
...
class Sprite(pygame.sprite.Sprite):
    ...
    def get_sprite(name: str, scale_value: float = 1, delay: int = 100, run: bool = True):
        path = os.path.join(_BASE, 'decompilation', 'sprite', f'{name}.json')
        with open(path) as f:
            data = json.load(f)
